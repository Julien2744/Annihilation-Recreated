...
#boss summoner check
execute if entity @r[predicate=annihilation:player_near_summoner] run execute positioned 211 -42.075 -1291.05 if data entity @e[type=interaction,tag=anni_summoner,distance=..1,limit=1] interaction run function annihilation:boss_summoner/check_player

#loop boss ai
execute if score #anniR_global anniR_spawned matches 1 run execute as @e[type=item_display,tag=aj.annihilation_recreated.root] at @s run function annihilation:boss/main_ai

#despawn the boss if there no player (only for structure boss)
execute if score #anniR_global anniR_boss_summoner matches 3 run execute positioned 211 -42.075 -1291.05 run execute if entity @e[type=item_display,tag=aj.annihilation_recreated.root,distance=..8,limit=1,scores={anniR_struct_spawned=1,anniR_playerCount=1..}] run execute as @e[type=item_display,tag=aj.annihilation_recreated.root,distance=..8,limit=1,scores={anniR_struct_spawned=1,anniR_playerCount=1..}] at @s unless entity @p[distance=..105] run function annihilation:boss/despawn