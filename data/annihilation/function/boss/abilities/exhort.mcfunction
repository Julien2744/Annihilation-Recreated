...
execute at @e[tag=anniR.target,tag=!anniR.immune,limit=1,distance=..10] rotated as @s run summon creeper ^ ^2 ^-0.5 {Tags:["anniR.exhort_launch"],NoGravity:1b,Silent:1b,Invulnerable:1b,PersistenceRequired:1b,NoAI:1b,ExplosionRadius:-1b,Fuse:0,ignited:1b,attributes:[{id:"minecraft:generic.scale",base:0}]}

#effect
effect give @e[tag=anniR.target,tag=!anniR.immune,limit=1,distance=..10] hunger 3 10
